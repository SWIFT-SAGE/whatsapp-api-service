...
export interface IMessageLog extends Document {
  ...
  type: 'text' | 'image' | 'audio' | 'video' | 'document' | 'location' | 'contact' | 'sticker' | 'gif' | 'chat' | 'interactive' | 'ptt';
  from: string;
  to: string;
  content?: string;
  mediaUrl?: string;
...
const messageLogSchema = new Schema<IMessageLog>({
  ...
  type: {
    type: String,
    enum: ['text', 'image', 'audio', 'video', 'document', 'location', 'contact', 'sticker', 'gif', 'chat', 'interactive', 'ptt'],
    required: true,
    index: true
  },
  from: {
